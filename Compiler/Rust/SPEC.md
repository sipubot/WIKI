...
```rust
//러스트 에서는 예약어 들을 변수나 함수명으로 지정할수 있게 만들어 준다
//r# 이 예약어를 치환해주는 역활.
pub fn r#if () {
}
...
```rust
pub fn str_to_string ( _v :&str) -> String {
    _v.to_string()
}

pub fn string_to_str ( _v : String ) -> &str {
    //* -> pointer has String to str(fixed size string)
    &*_v
}
...
- 탐색순서를 반드시 알아야만 동작을 이해할수 있다. (stack에서 꺼내올 순서대로 node 체크하게 되는 것을 이해할것
- (당연한 이야기 이지만 이진 트리 이므로 stack에 쌓이는 속도가 pop 속도보다 무조건 빠르므로 완전 탐색이 이루어 진다)
